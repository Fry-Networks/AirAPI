...
class PebbleClient {
  ...
  static async createClient(clients: PebbleClients, ObjectId: string) {
    ...
    setInterval(async () => {
      ...
      this.syncData(ObjectId, imei, clients, miner_key);
    }, 600000);
  }

  static async syncData(obj_id: string, imei: string, clients: PebbleClients, miner_key: string) {
    const data = (await PebbleApi.getPebbleDataByImei(imei))?.pebble_device_record[0];
    const client = clients.get(obj_id);
    console.log('Pebble syncData : ', data, client);
    if (data && client && data.timestamp !== client.last_data) {
      clients.set(obj_id, {
        imei,
        last_data: data.timestamp
      });
      this.saveData(data, imei, miner_key);
    }
  }

  static async saveData(data: PebbleData, imei: string, miner_key: string) {
